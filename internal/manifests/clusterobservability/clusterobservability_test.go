...
func TestAgentVolumesAndMounts_OpenShiftConditional(t *testing.T) {
	const kubeletCAName = "kubelet-serving-ca"

	t.Run("non-openshift omits kubelet-serving-ca", func(t *testing.T) {
		volumes := agentVolumes(false)
		mounts := agentVolumeMounts(false)
	...
	t.Run("openshift mounts kubelet-serving-ca as HostPathFile", func(t *testing.T) {
		volumes := agentVolumes(true)
		mounts := agentVolumeMounts(true)
	...
	t.Run("base host volumes are present in both distros", func(t *testing.T) {
		baseHostMounts := []string{"host-dev", "host-etc", "host-proc", "host-sys", "var-log-pods"}
		for _, isOS := range []bool{false, true} {
			names := volumeNames(agentVolumes(isOS))
			for _, want := range baseHostMounts {
				assert.Contains(t, names, want, "openshift=%v missing %s", isOS, want)
			}
			assert.Equal(t, len(agentVolumes(isOS)), len(agentVolumeMounts(isOS)),
