...
// buildExportersConfig wraps the user-provided exporter AnyConfig as the
// otlphttp exporter. The operator does not introspect the schema; whatever the
// user puts under `spec.exporter` is what the collector receives. Validation
// (presence of an endpoint, etc.) is performed by the validating webhook.
